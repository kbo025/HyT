<?php
namespace Navicu\InfrastructureBundle\Controller\Ascribere;
...
use Navicu\Core\Application\UseCases\Ascribere\RegisterTempProperty\RegisterTempPropertyCommand;
use Navicu\Core\Application\UseCases\Ascribere\RegisterTempServices\RegisterTempServicesCommand;
use Navicu\Core\Application\UseCases\Ascribere\AcceptTermsAndConditions\AcceptTermsAndConditionsCommand;
use Navicu\Core\Application\UseCases\Ascribere\AdvanceSection\AdvanceSectionCommand;
use Navicu\Core\Application\UseCases\Ascribere\AddPaymentInfo\AddPaymentInfoCommand;
use Navicu\Core\Application\UseCases\Ascribere\Rooms\SaveTempRoom\SaveTempRoomCommand;
use Navicu\Core\Application\UseCases\Ascribere\Rooms\SelectTempRoom\SelectTempRoomCommand;
use Navicu\Core\Application\UseCases\Ascribere\Rooms\DeleteTempRoom\DeleteTempRoomCommand;
use Navicu\Core\Application\UseCases\Ascribere\Images\RegisterTempImage\RegisterTempImageCommand;
use Navicu\Core\Application\UseCases\Ascribere\Images\RegisterTempFavoritesImages\RegisterTempFavoritesImagesCommand;
use Navicu\Core\Application\UseCases\Ascribere\Images\DeleteTempFavoriteImage\DeleteTempFavoriteImageCommand;
use Navicu\Core\Application\UseCases\Ascribere\Images\DeleteTempImage\DeleteTempImageCommand;
use Navicu\Core\Application\UseCases\Ascribere\Images\EditTempImage\EditTempImageCommand;
use Navicu\Core\Application\UseCases\Ascribere\Images\SortImages\SortImagesCommand;
use Zend\Serializer\Adapter\Json;

use Navicu\Core\Application\UseCases\AscribereDesktop\ValidateData\ValidateDataCommand;
use Navicu\Core\Application\UseCases\AscribereDesktop\CreateTempOwner\CreateTempOwnerCommand as CreateOwnerDesktop;

class AscribereController extends Controller
{
    ...
    /**
     *   esta accion envia al cliente la plantilla de la seccion que tiene pendiente o de la que 
     *   solicite
     *
     *   @author Gabriel Camacho <kbo025@gmail.com>
     *   @version 06/08/2015
    ...
    public function registerAction($slug, $level)
    {
        ...
        switch($level)
        {
            ...
            case 'status':
                //die(var_dump($tempOwner->getValidations()));
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            if ($this->get('SessionService')->isLoggedIn('ROLE_TEMPOWNER')) {
                ...
                if ($serviceFormTempOwner->haveAccess($data['slug'])) {
                    ...
                    $command->set('is_admin',$serviceFormTempOwner->isAdmin());
                    $response = $this->get('CommandBus')->execute($command);
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            if ($this->get('SessionService')->isLoggedIn('ROLE_TEMPOWNER')) {
                ...
                if ($serviceFormTempOwner->haveAccess($data['slug'])) {
                    ...
                    $command->set('client_ip',$request->getClientIp());
                    $command->set('is_admin',$serviceFormTempOwner->isAdmin());
                    $response = $this->get('CommandBus')->execute($command);
                    return new JsonResponse($response->getArray(), $response->getStatusCode());
                } else {
    ...
    public function saveTempRoomFormAction($slug, $index)
    {
        ...
        if ($serviceFormTempOwner->haveAccess($tempOwner)) {
            ...
            return $this->render(
                'NavicuInfrastructureBundle:Ascribere/PropertyForm:formRegisterRoom.html.twig',
                [
                    "rooms"=>json_encode($response),
                    'slugTemp' => $slug,
                    'basicQuota' => $tempOwner->getPropertyForm('basic_quota')
                ]
            );
        }
    ...
    public function desktopRegisterAction(Request $request)
    {
        $data = json_decode($request->getContent(),true);



        $command = new ValidateDataCommand($data);

        $response = $this->get('CommandBus')->execute($command);

        //die(var_dump($response->getArray()));

        if($response->getStatusCode() == '200') {

            $tempData = array();
            $tempData['data'] = $response->getArray();
            $tempData['imagemanagerResponse'] = $imagemanagerResponse = $this->container->get('liip_imagine.controller');
            $tempData['request'] = $this->getRequest();


            $command = new CreateOwnerDesktop($tempData);
